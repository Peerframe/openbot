...
| 已关闭引擎 | 持久命令不能自行重启已结束的工作流；另行显式恢复前，Action 保持 unknown |
...
目前公开 Owner 核对入口覆盖仍在运行的参考工作流；已关闭历史的运维恢复仍须单独处理。文件配额、回收及存储耐久性仍未完成。

当前固定流程候选在开发引擎与 PostgreSQL/mTLS 引擎各通过 15 项场景，包括旧引擎冷备份恢复后重新通知已送达、未完成的命令。外部服务为受控假件，不是 Linux/runsc 隔离验收。
