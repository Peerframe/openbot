...
旧 TypeScript 业务 Server 已退役。使用新的 Compose 项目；已有卷和安装保留，不会自动转换。

默认容器打包真实 Python 产品 API、构建后的 Web、保留的 PostgreSQL 迁移器和文档解析器，
不依赖已退役的 TypeScript 业务 Server 或测试 oracle。
...
该历史记录对应当时的检查状态；PR98 的全部门禁随后通过。当前退役版本仍需其独立 CI 结果，配置 Temporal 与部署需显式执行。
