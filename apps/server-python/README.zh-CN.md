...
71 项数据库/HTTP 检查覆盖读取、过期/撤权、只读事务拒绝写入、精确模式历史、非法旧 Bot 状态、并发持久限流、
...
没有显式夹具时普通包测试跳过 71 项集成，跳过不算验收。固定版本仍有两项上游测试客户端弃用提示。
...
202 和 `steering`。这表示指令已记录，尚不表示已执行；后续完成事务必须核对全部已提交指令的 ID。
