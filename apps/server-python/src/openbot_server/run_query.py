...
_TEXT_COLUMNS = ("id", "parent_run_id", "root_run_id", "delegated_by_bot_id", "channel_id", "bot_id",
                 ...
                 "result_summary", "error_message", "error_code")
_COLUMNS = ",".join((*_TEXT_COLUMNS, "model_usage", "created_at", "updated_at"))
...
RUN_QUERY = (
    f"WITH recent AS MATERIALIZED (SELECT {_COLUMNS} FROM runs WHERE channel_id=%s "
    "ORDER BY created_at DESC,id COLLATE \"C\" DESC LIMIT 50), "
    f"sized AS (SELECT recent.*,sum({_BYTE_SIZE}+coalesce(octet_length(model_usage::text)::bigint,0)) "
