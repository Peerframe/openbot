...
| `plugin-receipts` | **任务详情**，再展开 **已终结回执** 和 **时间与插件版本** | 已取消任务仍在操作按钮前显示未知外部结果，并提供已收答复样本；可核对调用 ID、审批事实和时间，不提供调用重试。关闭详情，选择 **模拟回执读取失败** 后重新打开，可查看固定错误提示；**恢复回执读取** 与 **刷新回执** 仅执行读取。 |
...
回执场景是只读内存样本，不是持久账本，也不是外部效果的独立证明。其未知记录不会被转换成
写入成功。原有六个场景与公开演示对已知合成任务返回空回执。轮询边界和故障检查见
[回执界面研究](research/plugin-call-receipts-ui.zh-CN.md)。
