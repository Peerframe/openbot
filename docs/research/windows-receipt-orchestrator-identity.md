...
## Main-module readiness after process creation
...
- [Microsoft `Process.Refresh`](https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.process.refresh?view=net-10.0) invalidates cached process information. Reviewed **dotnet/runtime v10.0.9**, Git ref `5eaa18a9f3398d54ba9b8c0974d88171663be892`: [`Process.Windows.cs`](https://github.com/dotnet/runtime/blob/v10.0.9/src/libraries/System.Diagnostics.Process/src/System/Diagnostics/Process.Windows.cs) reads the main module through `NtProcessManager.GetFirstModule`; [`Process.cs`](https://github.com/dotnet/runtime/blob/v10.0.9/src/libraries/System.Diagnostics.Process/src/System/Diagnostics/Process.cs) `Refresh` and Windows `RefreshCore` clear metadata without disposing/replacing the held process handle. `Close`, unlike `Refresh`, releases it. The Windows main-module getter itself does not use the cached module collection, so waiting for readiness is the essential correction; refresh also keeps exit observations current.
...
Selected: the standard `Process` API on the same held object, with a five-second monotonic deadline and at most 50 ms between readiness observations. Return only after PID, exact UTC start time, nonempty module path and a live process are established. Refresh may not change the held handle. Retry only null/empty module paths; exit, permission errors and other property exceptions fail immediately. A missing path at the deadline remains a failure. Never read identity from the receipt to repair the held observation, reacquire by bare PID during retries, add native interop, or relax strict equality.
...
Local verification: the complete receipt-identity script passed on macOS PowerShell 7.5.4 / .NET 9.0.10, including all 30 assertions across ten real child lifetimes and the existing JSON/multiple-Node-path checks. The Windows Node/WinPS preflight was explicitly skipped on that host. This run does not establish that Windows returned a transient null module, nor does it replace the pending native installed-runtime CI.
