...
| Server-owned native Agent loop | ai 7.0.93, @ai-sdk/openai 4.0.60, @ai-sdk/anthropic 4.0.49; compared OpenAI Agents JS v0.17.0 and the pinned Hermes runtime | Apache-2.0; MIT | Reuse released ToolLoopAgent with strict channel read tools, bounded iteration and official provider HTTP. Owner opt-in, PostgreSQL claims/replies/audit and settings revocation remain in OpenBot. No upstream source copied; see [research](research/native-agent-loop.md) and [runtime](NATIVE_AGENT.md). |
| Native Agent public sources and reports | ipaddr.js 2.5.0 / dc552827; html-to-text 10.0.1 / 1c39d988; Node HTTPS and existing AI SDK/artifact storage | MIT; Node.js license; Apache-2.0 | Reuse address classification and bounded text extraction with a thin task-URL adapter, pinned public connections and transactional Markdown report publication. No upstream source copied; see [research](research/agent-research-artifacts.md). |
| Native task cancellation, profile and usage | Node AbortController; PostgreSQL 17 row locks; ai 7.0.93 / 6359fd58; React 19.2.8 | Node.js; PostgreSQL; Apache-2.0; MIT | Extend existing Server-owned Run lifecycle with durable cancellation before abort, bounded assigned-Bot context, provider-reported usage and explicit new-task resubmission. No upstream source copied; see [research](research/agent-execution-experience.md). |
| Desktop unified toolbar and view history | React 19.2.8; Apple toolbar and WAI-ARIA guidance reviewed 2026-09-05; compared React Router 8.3.0 / 2edaca7 and react-resizable-panels 4.9.0 | MIT; Apple/WHATWG/W3C terms | Reuse native controls and React state for a fixed toolbar, independent panel toggles and bounded history of authorized view selections. Preserve the authenticated workspace while Settings is shown; changing Owner/Server or logout ends its lifetime. No URL router, drag-resize dependency or Codex source/assets incorporated. See [research](research/desktop-navigation-continuity.md). |
| Desktop channel drafts and send continuity | Existing React 19.2.8, native HTML textarea and CSSOM View; compared react-textarea-autosize 8.5.9 | MIT; WHATWG/W3C terms | Reuse external-store snapshots and native geometry for a bounded workspace-memory cache, channel-specific draft revisions, pending-send ownership and reading-position restoration. Text never enters localStorage; newer edits survive completion, and ambiguous failures are not automatically retried. No new dependency or source copied; see [research](research/desktop-conversation-continuity.md). |
| Channel-first Desktop inspector | Native HTML details/summary, React 19.2.8 / 1dd4ecb, and OpenBot authenticated snapshot at 14e41ae78a35cd6a6b52de25a4e60300c2fe993d | WHATWG terms; MIT | Project current-channel approvals, active tasks and recent results before collapsed workspace totals. Validate channel/run associations and retain pending approvals beyond the recent-run sample. No added accounting or execution authority; missing Token usage remains unavailable. No source copied; see [research](research/desktop-contextual-inspector.md). |
| Native Desktop navigation menus | Electron 44.2.0 / aa650d7 public Menu/role APIs; Apple menu organization reviewed 2026-09-05 | MIT; Apple API terms | Reuse Chinese native menus and standard editing/window roles. Six fixed presentation commands and an exact four-boolean trusted-frame snapshot coordinate menu availability with the renderer; lifecycle, focus and local-origin guards run again at selection time. No generic IPC, global shortcut, Reload or Developer Tools menu. No source copied; see [research](research/desktop-native-navigation-menu.md). |
...
| Browser computer | [CopilotKit/OpenBot `agent-computer` `257c1280`](https://github.com/CopilotKit/openbot/tree/257c1280d684089be9adb0b35cce262efc7064bf/agent-computer) | MIT | Use the token-protected HTTP surface through a thin Provider adapter. The upstream process stays separate; no control plane is copied. Reuse snapshot/control/click APIs for the default-off reviewed-click slice; see [research](research/controlled-browser-click.md). |
...
| Realtime overload recovery | [Hono streaming `eebdf7be`](https://github.com/honojs/hono/blob/eebdf7be39abf0a872671835ccce0c4f03ea497a/src/utils/stream.ts) | MIT | Keep Hono's backpressure-aware writer and add only the missing OpenBot per-subscriber policy: a 128-event bound, abort on overflow, and authoritative snapshot recovery. No upstream source was copied. |
...
| macOS Worker Host launchd boundary | macOS 13+ `SMAppService`, LaunchAgent, `BundleProgram`, and current launchd contracts; rejected [`node-mac` `1.0.1` / `2bf9e64f`](https://github.com/coreybutler/node-mac/tree/2bf9e64f637f3ab1f036bf69c1205d0f6c227e7b), [`emorydunn/LaunchAgent` `0.3.0` / `9f6d33ab`](https://github.com/emorydunn/LaunchAgent/tree/9f6d33abfa4a0a45a877e9b884391981c1f4152b), [Homebrew `6.0.21` / `56014701`](https://github.com/Homebrew/brew/tree/560147012b9678b42ef5e83b690f0895552d1366), and [Swift Service Lifecycle `2.11.0` / `9829955b`](https://github.com/swift-server/swift-service-lifecycle/tree/9829955b385e5bb88128b73f1b8389e9b9c3191a) | Apple platform/API terms; MIT declaration with reviewed license defect; MIT; BSD-2-Clause; Apache-2.0 | Use a user-approved app-bundled LaunchAgent for the dedicated standard account, one fixed `BundleProgram` and `--worker-host` mode, conditional crash restart, private umask, and bounded launchd stop. The native dual-mode main executable, registration UI, fixed child supervisor, plist contract tests, native Swift tests, and arm64 candidate build are implemented. Developer ID, registration lifecycle, and real-device evidence remain external gates. Do not add a root daemon, arbitrary command, legacy plist installer, package-manager dependency, second restart loop, credential-bearing plist/argv, or desktop permission. No candidate source is copied; see [research evidence](research/macos-worker-host-launchd.md). |
| macOS Worker Host configuration and Keychain | Apple Security data-protection Keychain, access-group and restricted-entitlement contracts; Node.js [`22.22.2` / `2645dc73`](https://github.com/nodejs/node/tree/2645dc73720b1b4f27c49f395d3c66025ce126cc); [Zod `4.5.4` / `e8e206fa`](https://github.com/colinhacks/zod/tree/e8e206fa33ac5fe7ce20a2beb12d57b1cb3df653); rejected [SwiftSecurity `2.0.0` / `c26f18ad`](https://github.com/dm-zharov/swift-security/tree/c26f18adbb20f9e4e810d3ba7553e095a74ea2c0), [KeychainSwift `24.0.0` / `5e1b02b6`](https://github.com/evgenyneu/keychain-swift/tree/5e1b02b6a9dac2a759a1d5dbc175c86bd192a608), [`@napi-rs/keyring` `2.0.0` / `f3449416`](https://github.com/Brooooooklyn/keyring-node/tree/f3449416a1b4bf11b0570f0a49395aacc84c8608), [Cosmiconfig `10.0.1` / `219805f4`](https://github.com/cosmiconfig/cosmiconfig/tree/219805f445a26a6190d3a46ad43c8b5c3f0a172e), and [Convict `6.2.5` / `517840e1`](https://github.com/mozilla/node-convict/tree/517840e19681be3ddc1c8f5d47d0e6c781795cf2) | Apple platform/API terms; Node.js license; MIT; MIT; MIT; MIT; MIT; Apache-2.0 | Query one exact non-synchronizing, device-only item through Apple Security inside the signed app executable. Reuse opened-handle Node core and strict Zod for one fixed non-executable public JSON config. Strict config, Server-bound envelopes, direct Security queries, access-group derivation, one-shot `stdio-v3` handoff, and bounded process-group supervision are implemented and pass Node/Swift tests. A distribution-authorized profile and locked/unlocked real Keychain evidence remain pending. General wrappers, native addons, `/usr/bin/security`, and config search/merge libraries expose broader callable or mutable surfaces without enforcing the fixed invocation. No upstream source is copied; see [research evidence](research/macos-worker-host-config-and-keychain.md). |
| macOS Worker Host registration and package | Apple `SMAppService`, Security, Developer ID, `codesign`, `productbuild`, `pkgutil`, `notarytool`, `stapler`, and Gatekeeper; rejected SwiftPkg [`0.4.1` / `ad0007e7`](https://github.com/codecarton/swiftpkg/tree/ad0007e7263360950c17cca6d0c4b0d3084a9b13), retired MunkiPkg [`7d57d075`](https://github.com/munki/munki-pkg/tree/7d57d0750bd6787838322b7abf02693da033d584), and Mackage [`1.0.0` / `766db3a8`](https://github.com/tactcomplabs/mackage/tree/766db3a83276d0cc5cd2260471b334295960171a) | Apple platform/API terms; Apache-2.0; BSD-style/`NOASSERTION`; Apache-2.0 | Use direct Apple tools for one app-only package with no installer scripts. Candidate staging, exact Node pins, Mach-O architecture checks, entitlement/profile/Team binding, inside-out signing, post-sign manifest refresh, AppleDouble rejection, notarization/stapling/Gatekeeper gates, and local arm64 unsigned/ad hoc mechanics are implemented. This Mac has no distribution identities or notary profile, so signed/notarized and install lifecycle claims remain pending. No candidate source is copied; see [research evidence](research/macos-worker-host-package-and-registration.md). |
...
- Employee export filenames follow RFC 6266's advisory-name safety guidance and use the exact-pinned
  MIT `filename-reserved-regex` 4.0.1 predicate for Windows device names. Because that published
...
- File-backed artifacts reuse `write-file-atomic` for fsync, rename, and failed-temporary cleanup;
  final files retain verified `0600` permissions on POSIX. Windows storage lifecycle coverage does
  not claim ACL protection from emulated mode bits.
...
- An Owner can issue a short-lived single-use token for one Node id. A real PostgreSQL concurrency
  ...
  before opening the WebSocket and refuses later group/other-readable POSIX permission drift.
...
- The standalone Provider scenario runner is implemented; integrations still need Provider-specific
  hermetic scenarios and repeatable real-device CI before a platform is marked supported or certified.
...
- Node bootstrap uses a copyable bearer credential. It defaults to an Owner-only file; a dedicated
  Linux login-session Secret Service adapter is contract-tested but lacks real-device evidence.
  Non-exportable proof-of-possession keys, Windows/macOS keyrings, rotation, mTLS, replay
  protection, and persisted reconciliation remain open.
...
Native asynchronous collaboration continues AI SDK 7.0.93 / `6359fd58fe68eaade096b5d923bac26de84ca3bd` (Apache-2.0) with PostgreSQL 17 locks and Server-owned RunEvents. No upstream source copied. The bounded nonblocking join, authenticated steering and real stream adapter review is in [async collaboration](research/async-collaboration.md). Reactions/member removal reuse PostgreSQL transactions and React 19.2.8 portals; Matrix specification v1.17 is a semantics reference only, not a federation dependency. See [channel interactions](research/channel-interactions.md), including request-limit isolation and accessible side-of-bubble controls.
...
## Dependabot patch pins (2026-09-11)

Exact-pinned patch consolidation (Dependabot #24, #25, #26, #28; Vitest 5 #27 excluded):

| Package | Pin | License | Role |
| --- | --- | --- | --- |
| hono | [`4.13.7` / `eebdf7be`](https://github.com/honojs/hono/tree/eebdf7be39abf0a872671835ccce0c4f03ea497a) | MIT | Server HTTP; includes `hono/jsx` XSS fix GHSA-hxh3-vqpv-xpqv |
| @biomejs/biome | `2.5.12` | MIT OR Apache-2.0 | Root lint/format DevDependency only |
