...
当前分支：`codex/architecture-migration`；最新已验证的 S3 代码为 `b0db90d`，S2 外壳代码为 `b2de930`。后续先读本交接和下列文件；只有出现具体失败时再查旧日志。
...
下一步 S3 先读 `docs/research/work-temporal-journey.md` 最后一节、`apps/agent-runtime-python/src/openbot_agent_runtime/{executor.py,sdk_ports.py}` 和 `experiments/work-journey/workflow_worker.py`。只有连接产品派发时才读 `apps/server-python/src/openbot_server/work_handoff.py`。保留无关的模型服务未提交文件与 TASK020 未跟踪候选。
