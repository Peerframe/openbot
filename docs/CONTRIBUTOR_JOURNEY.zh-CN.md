...
完整 `--with-node` 流程已在 macOS arm64、Node 26.0.0、npm 11.12.1 和固定 PostgreSQL Docker 镜像下通过，包含默认路径的全部断言与可选 Node 断言。七项定向 fixture 测试和 `npm run check` 均通过。在另一份干净 fixture 中，于数据库和开发进程启动后发送 SIGTERM：驱动以非零状态退出并指出失败阶段，未遗留 smoke 私有目录、所有权容器或 3001/5173 监听服务。这是本地证据；该工作包没有修改或单独执行已有 hosted Node 22/npm CI 固定版本。
