...
This is an unsigned development candidate. The current `618e399` candidate packages all44 canonical migrations and63 pinned dependencies. Fresh staged and packaged tests passed Owner login, actual API/PostgreSQL startup, preserved data across restart, parent-death cleanup and unsafe-directory refusal. All162 Python source modules, lifecycle scripts, lock and SQL bytes match the checkout; the packaged controllers match the compiled source. Two actual bundled Worker starts connected to an owned mTLS Temporal service and exposed fresh Workflow/Activity pollers, followed by complete cleanup. [Current evidence](../experiments/work-journey/evidence/desktop-preview-schema44.json) records these checks.

Earlier canonical41 native GUI/Keychain evidence and canonical43 package evidence remain pinned to their own artifacts: [native result](../experiments/work-journey/evidence/desktop-native-keychain.json), [previous refresh](../experiments/work-journey/evidence/desktop-preview-refresh.json). The current artifact has not repeated native GUI/Keychain acceptance or complete packaged model execution. It is not installed, signed or released; normal packaging still selects the existing backend.
