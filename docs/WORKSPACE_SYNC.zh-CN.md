...
## 官方 Web/Desktop 客户端校准

共享 `useWorkspaceState` hook 保留最近一次成功 GET 的 `counts.activeRuns`。旧任务事件及成功的
任务/审批响应仍立即更新实体，但不对全局计数做加减。近期列表不能确定页面外任务原先是否已计入，
GET 也可能已经包含刚收到的事件。

任务投影请求一次合并后的权威补读。事件触发的读取最多每秒启动一次，仅有一个当前请求和一个
待校准标记。成功读取期间到达的事件会安排后续新读取；重放实体 journal 不触发读取。因此有限
事件突发会自行结束，不增加轮询或自动刷新循环。显式刷新和重连保留取消旧请求、启动新读取的行为，
并吸收已有待校准标记。

读取失败时保留旧快照和即时实体更新，通过现有错误界面报告失败，不自动继续重试；后续事件、
显式刷新或重连可再次读取。卸载会清除计时器并取消/作废请求。全局计数在校准完成前可能短暂落后于
实体更新。当前 ContextRail 指标明确表示已加载的近期记录，保持该范围，不将其重新命名为全局总数。
官方客户端仍使用 GET 和旧事件流。

确定性测试使用真实共享 hook 和已登录工作区组件，无需账户凭据或付费模型。参见
[实现研究](research/workspace-authoritative-counts.zh-CN.md)。
...
此契约限于单 Server。原有 GET 和事件客户端保持兼容，Web 实体仍即时投影，全局活跃计数按上述规则校准。
Desktop 的通用 `/api/v1/*` 代理可以转发此路由，但事件流生命周期管理器尚无快照连接的替换/清理
槽位，正式 renderer 也未订阅。接入 Desktop 前应先补充并验证这部分生命周期。大工作区分页、
持久版本日志和多 Server 分发仍需独立设计，不能把 sequence 当作这些能力。
