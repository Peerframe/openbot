...
共享 `useWorkspaceState` hook 在 GET 成功后订阅版本 1 全量快照流。`counts.activeRuns` 始终
来自权威 GET 或完整流帧。旧任务事件及成功的任务/审批响应仍立即更新实体，但不对全局计数加减。
近期列表不能确定页面外任务原先是否已计入，权威读取也可能已经包含收到的事件。

GET、全量流帧和旧事件/操作响应之间没有可共同比较的版本号。因此，官方客户端在**每次即时实体
投影和每次 GET 开始前**关闭并作废快照订阅。GET 期间到达的投影记入 journal，再覆盖到读取结果上；
期间只要有投影，就必须再补读一次。在后续补读完成前不重新订阅。仅当成功 GET 没有待补读标记时，
才创建新的快照订阅轮次。已关闭轮次的晚到帧、旧 GET 响应及旧事件连接回调不能覆盖当前状态。
迟到的操作 Promise 响应同样会建立新的投影屏障。

所有即时实体投影都请求合并后的权威补读。事件触发的读取最多每秒启动一次，仅一个当前请求和一个
待补读标记。重放 journal 不触发新读取。显式刷新与工作区重连可取消并替换当前 GET。有限事件突发
会自行结束，但**持续旧事件可能使快照流一直关闭，并持续每秒执行一次合并 GET**。本次接入不宣称
减少 GET 请求。

GET 失败时保留旧数据和即时投影，通过现有错误界面报告失败；不自动重试 GET，也不重新开流。
后续实体事件、显式刷新或旧工作区流重连可重试。仅快照流断开时保留数据，通过单个两秒计时器重连。
仅打开响应头不代表“已同步”，必须收到完整有效帧。连续 35 秒没有更新的有效帧会关闭并替换连接。
版本、包裹结构、集合形状不兼容，帧超限或同连接的 streamId 改变时停止该订阅并显示固定错误，可
显式刷新重试。流恢复不会清除独立的操作/GET 错误。工作区同步指示同时反映全量快照流和旧工作区
流；频道保留独立连接状态以及文字/进度传递。

卸载会清除计时器、关闭订阅、作废轮次并取消 GET，不等待卡住的读取。Owner/Server 变化会重新挂载
已登录工作区。员工资料通知继续通过旧事件流传递。ContextRail 指标仍表示已加载的近期记录，不改名
为全局总数；权威计数在补读结束前可能短暂落后于即时实体投影。

确定性测试使用真实共享 hook、已登录工作区组件和受控 EventSource，无需凭据或付费模型。参见
[顺序与生命周期研究](research/official-workspace-snapshot-stream.md) 和先前的
[全局计数研究](research/workspace-authoritative-counts.zh-CN.md)。
...
Desktop 通用 `/api/v1/*` 代理现在独立管理旧工作区、全量快照和当前频道三个上游连接槽。替换某个
连接只取消该槽；主框架导航、renderer 终止、窗口关闭、Server/配置清理和应用退出同步取消全部三个槽。
Node 测试使用真实 AbortSignal，覆盖实际注册的窗口事件与连接控制器，也覆盖上游响应头一直未返回的情况。
这是桥接夹具验证，不代表已安装 Electron 或远程 Server 的实测。不能仅依赖 renderer EventSource.close
把取消信号穿过 Electron 重建的代理响应；主进程注册表负责限制并清理上游连接。大工作区分页、持久
版本日志和多 Server 分发仍需独立设计，不能把 sequence 当作这些能力。
