...
try {
  foreach ($mode in @('progress', 'stalled', 'bounded', 'nonzero')) {
    ...
    try {
      $maximumMs = if ($mode -eq 'bounded') { 2000 } else { 10000 }
      ...
      $expected = switch ($mode) { 'stalled' { 'stalled' }; 'bounded' { 'duration-limit' }; default { 'completed' } }
